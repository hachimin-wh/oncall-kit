...
**个人理解**：**值班 Agent 系统 = oncall-kit + 运行环境**，oncall-kit 通过规则、流程、排查手册和历史经验定义值班业务 SOP，运行环境提供模型、工具调用与任务调度。Agent 在约定边界内调查、提出建议并验证结果，人负责关键决策与修复操作。

## 2. 业务场景与职责划分

项目覆盖以下四类场景，由对应 Skill 执行，第一行是初次的接入配置，后三行是接入完成后的日常运行。
...
四个场景都是同一个模式：**Agent 负责取证、分析，人负责关键决策和修复操作。**
